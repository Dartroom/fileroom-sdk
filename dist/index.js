...
async function connectWS(url) {
  return new Promise(function(resolve, reject) {
    var server = new WebSocket__default.default(url);
    server.onopen = () => {
      resolve(server);
    };
...
var FetchHttpClient = class extends HttpClient {
  constructor(config) {
    ...
    this._fetch = browserOrNode.isBrowser ? window.fetch || fetch__default.default : fetch__default.default;
    this._isLegacyBrowser = browserOrNode.isBrowser && !window.fetch;
    if (config) {
      let headers = {
...
var IpfsApi = class extends BaseApi {
  ...
  async get(cid, options) {
    ...
    if (this.createHttpRequest._isLegacyBrowser) {
      ...
      stream = browserOrNode.isBrowser ? response.toStream(() => {
      }) : response.toStream(() => {
      });
      return stream;
    }
...
var UploadApi = class extends eeTs.EventEmitter {
  ...
  async start(file, options) {
    ...
    let upload = new tusJsClient.Upload(file, {
      endpoint: this._url,
  ...
  async setfileMeta(file) {
    let sha = "";
    if (browserOrNode.isBrowser && (typeof File !== "undefined" && file instanceof File || typeof Blob !== "undefined" && file instanceof Blob)) {
      this._uploadOptions["filetype"] = file.type;
      this._uploadOptions["name"] = file.name;
      this._uploadOptions["size"] = file.size.toString();
      let hash = crypo__default.default.createHash("sha256");
    ...
    if (file instanceof stream.Stream) {
      if (file.hasOwnProperty("path") && file.path) {
        ...
        let mimeType = mime__default.default.lookup(file.path);
        if (!mimeType)
